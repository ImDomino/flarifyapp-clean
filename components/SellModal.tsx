...
import { X, TrendingDown, Info, AlertCircle, Loader2, Zap, CheckCircle } from "lucide-react";
...
export function SellModal({ isOpen, onClose, position, currentPrice, onSuccess }: SellModalProps) {
  const { authenticated, login } = usePrivy();
  const { sellShares } = usePlaceOrder();

  const [sellAmount, setSellAmount] = useState<string>("");
  const [sellPrice, setSellPrice] = useState<string>((currentPrice ?? position.avgPrice).toFixed(2));
  ...
  useEffect(() => {
    ...
    const fetchBestBid = async () => {
      try {
        const res = await fetch(`/api/polymarket/price?token_id=${encodeURIComponent(position.asset_id)}`);
        if (!res.ok || cancelled) return;
        const data = await res.json();
        if (!cancelled && data.bestBid != null && data.bestBid > 0) setBestBid(data.bestBid);
      } catch { /* silent */ }
      finally { if (!cancelled) setLoadingBest(false); }
    };
    fetchBestBid();
  ...
  const handleSell = async () => {
    ...
    if (!sellAmountNum || sellAmountNum <= 0 || !sellPriceNum || sellPriceNum <= 0) { setError("Please enter valid amount and price"); return; }
    if (sellPriceNum < MIN_PRICE || sellPriceNum > MAX_PRICE) { setError("Price must be between 0.01 and 0.99 (1¢–99¢)"); return; }
    if (exceedsPosition) { setError(`Cannot sell more than ${maxShares.toFixed(2)} shares`); return; }
    if (isBelowMinimum) { setError(`Minimum order size is ${MIN_SHARES} shares`); return; }
    ...
    try {
      await sellShares(position.asset_id, sellPriceNum, sellAmountNum, position.negRisk ?? false);

      setSuccessMsg(`Selling ${sellAmountNum.toFixed(2)} shares at ${(sellPriceNum * 100).toFixed(1)}¢`);
      ...
      if (err.message?.includes("Size") || err.message?.includes("minimum")) errorMessage = `Minimum order size: ${MIN_SHARES} shares`;
      else if (err.message?.includes("not enough balance") || err.message?.includes("insufficient") || err.message?.includes("allowance")) errorMessage = "Insufficient shares or approvals missing.";
      else if (err.message?.includes("invalid signature")) errorMessage = "Signature error. Try logging out and back in.";
      else if (err.message?.includes("invalid price")) errorMessage = "Invalid price. Use between 0.01 and 0.99 (1¢–99¢).";
  ...
  const handleSetPercentage = (percent: number) => setSellAmount(((maxShares * percent) / 100).toFixed(2));
        ...
        <div className="w-full max-w-md bg-[#0a0a0a] border-2 border-white pointer-events-auto max-h-[90vh] overflow-y-auto relative">
          ...
          <div className="relative p-5 border-b border-zinc-800">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="font-black text-lg uppercase tracking-wider text-white">Sell Position</h2>
            </div>
            {position.question && <p className="text-xs text-zinc-500 font-bold line-clamp-2 pr-8 mt-1">{position.question}</p>}
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            {successMsg && (
              <div className="border-2 border-white bg-white/5 p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-black uppercase tracking-wider text-white">Sell Order Placed</p>
              ...
              <>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Shares to Sell</label>
                    <button onClick={handleSetMaxAmount}
                      className="text-xs text-zinc-400 hover:text-white font-bold uppercase tracking-wider">Max: {maxShares.toFixed(2)}</button>
                  </div>
                  <input type="number" value={sellAmount} onChange={(e) => setSellAmount(e.target.value)}
                    ...
                    min="0" max={maxShares} step="0.01" />
                  <div className="grid grid-cols-4 gap-px bg-zinc-800 mt-2">
                    {[25, 50, 75, 100].map((pct) => (
                      <button key={pct} onClick={() => handleSetPercentage(pct)}
                        className="py-2 text-xs font-black uppercase tracking-wider text-zinc-400 bg-[#111] hover:text-white hover:bg-[#1a1a1a] transition-colors">{pct}%</button>
                    ))}
                  </div>
                </div>
                ...
                {/* Sell price */}
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-2 block">Sell Price ($)</label>
                  <div className="relative">
                    ...
                    <input type="number" value={sellPrice} onChange={(e) => setSellPrice(e.target.value)}
                      ...
                      min={MIN_PRICE.toString()} max={MAX_PRICE.toString()} step="0.01" />
                  </div>
                  <p className="mt-1 text-[10px] text-zinc-600 uppercase tracking-wider font-bold">0.20 = 20¢ · Range: 1¢–99¢</p>
                </div>

                {/* Best Price */}
                {bestBid != null && bestBid > 0 && (
                  <button onClick={handleBestPrice}
                    ...
                    <Zap className="w-4 h-4" />Instant Sell: {(bestBid * 100).toFixed(1)}¢
                  </button>
                )}
                {loadingBest && (
                  ...
                  <div className="border border-zinc-800 bg-[#111] divide-y divide-zinc-800">
                    ...
                    <div className="flex justify-between items-center p-3">
                      ...
                      <span className={`text-sm font-mono font-bold ${pnl >= 0 ? "text-white" : "text-red-400"}`}>
                        {pnl >= 0 ? "+" : ""}${pnl.toFixed(2)}
                        <span className="text-zinc-600 ml-1">({pnlPercent >= 0 ? "+" : ""}{pnlPercent.toFixed(1)}%)</span>
                      </span>
                    </div>
                  </div>
                )}
                ...
                <button onClick={handleSell}
                  disabled={!sellAmountNum || sellAmountNum <= 0 || !sellPriceNum || sellPriceNum <= 0 || sellPriceNum < MIN_PRICE || sellPriceNum > MAX_PRICE || isProcessing || isBelowMinimum || exceedsPosition}
                  ...
                  {isProcessing ? (
                    <span className="flex items-center justify-center gap-2"><Loader2 className="w-5 h-5 animate-spin" />Placing sell order...</span>
                  ) : (
                ...
                {!authenticated && (
                  <p className="text-center text-xs text-zinc-600 uppercase font-bold tracking-wider">Sign in to sell positions</p>
                )}
