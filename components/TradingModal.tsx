...
export function TradingModal({
  ...
  const resolveTokenId = (): string | undefined => {
    return side === "yes" ? yesTokenId : noTokenId;
  };
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      alert(
        `✅ Order placed!\n\nOrder ID: ${orderId}\n\nYou ${actionLabel} ${shareCount.toFixed(2)} ${side.toUpperCase()} shares at ${displayPriceCents.toFixed(1)}¢`
      );
      onClose();
      setAmount("10");
      setError(null);
        ...
        <div className="w-full max-w-md bg-[#0a0a0a] border-2 border-white pointer-events-auto max-h-[90vh] overflow-y-auto relative">
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-px bg-zinc-800">
              <button
                onClick={() => { setSide("yes"); setError(null); }}
                className={`py-3 text-center text-sm font-black uppercase tracking-wider transition-colors ${
                  side === "yes" ? "bg-white text-black" : "bg-[#111] text-zinc-500 hover:text-white"
                }`}
              >
                Yes {Math.round(yesPrice * 100)}¢
              </button>
              <button
                onClick={() => { setSide("no"); setError(null); }}
                className={`py-3 text-center text-sm font-black uppercase tracking-wider transition-colors ${
                  side === "no" ? "bg-white text-black" : "bg-[#111] text-zinc-500 hover:text-white"
                }`}
              >
            ...
            <div className="grid grid-cols-2 gap-px bg-zinc-800">
              <button
                onClick={() => { setTradeType("buy"); setAmount("10"); setError(null); }}
                className={`py-3 text-center text-sm font-black uppercase tracking-wider transition-colors ${
                  tradeType === "buy" ? "bg-white text-black" : "bg-[#111] text-zinc-500 hover:text-white"
                }`}
              >
                Buy
              </button>
              <button
                onClick={() => { setTradeType("sell"); setAmount(""); setError(null); }}
                className={`py-3 text-center text-sm font-black uppercase tracking-wider transition-colors ${
                  tradeType === "sell" ? "bg-white text-black" : "bg-[#111] text-zinc-500 hover:text-white"
                }`}
              >
                Sell
              </button>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-[#111] border border-zinc-800 ${
                    tradeType === "buy" ? "pl-8" : "pl-4"
                  } pr-4 py-4 text-lg font-bold focus:outline-none focus:border-white transition-all text-white placeholder-zinc-700`}
                  min="0"
                ...
                {tradeType === "buy" ? (
                  <>
                    ...
                    <div className="flex justify-between items-center p-3">
                      ...
                      <span className={`text-sm font-mono font-bold ${profit > 0 ? "text-white" : "text-zinc-500"}`}>
                        ${profit.toFixed(2)}
                      </span>
                    </div>
                  </>
            ...
            {tradeType === "buy" && isBuyBelowMin && (
              <div className="border border-zinc-600 bg-[#111] p-3 flex items-start gap-2">
                ...
                <p className="text-xs text-zinc-400 font-bold">
                  {isBuyBelowMinUsd
                    ? `Minimum $${MIN_BUY_AMOUNT_USD.toFixed(2)} for BUY orders`
                    : `Minimum: ${MIN_SHARES} shares ($${minAmountByShares.toFixed(2)})`}
                </p>
              </div>
            )}
