...
export function MarketCard({ marketData, marketId }: MarketCardProps) {
  ...
  // Live prices + resolved status from API
  ...
  const [winner, setWinner] = useState<string | null>(null); // "Yes" | "No"
  ...
  useEffect(() => {
    ...
    const fetchLivePrice = async () => {
      try {
        ...
        // Resolved status from Gamma API
        if (data.resolved) {
          setIsResolved(true);
    ...
    fetchLivePrice();
    // No need to poll resolved markets frequently
    const interval = setInterval(fetchLivePrice, isResolved ? 600_000 : 120_000);

    return () => {
      cancelled = true;
      clearInterval(interval);
    };
  }, [marketData.yesTokenId, marketId, isResolved]);

  // Use live prices if available, otherwise cached
  ...
  return (
    <>
            ...
            <div className="flex-1 min-w-0">
              ...
              <h3
                className={`text-sm font-bold leading-relaxed ${
                  isResolved ? "text-zinc-500" : "text-white"
                }`}
              >
                {marketData.question}
              </h3>
            ...
            <div className="h-1.5 w-full bg-zinc-800 relative overflow-hidden">
              <div
                className={`absolute inset-y-0 left-0 transition-all duration-500 ${
                  isResolved ? "bg-zinc-600" : "bg-white"
                }`}
          ...
          <div className="px-4 sm:px-5 py-3 border-b border-zinc-800 bg-zinc-900/40">
            <div className="flex items-center justify-center gap-2.5 py-2">
              ...
              <span className="text-xs font-black uppercase tracking-wider text-zinc-500">
                Outcome:{" "}
                <span className={
                  winnerNormalized === "Yes" ? "text-emerald-500" : "text-red-400"
                }>
                  {winnerNormalized}
                </span>
              </span>
      ...
      {/* Trading Modal — only mount for active markets */}
      {!isResolved && (
        <TradingModal
